# Node.js and V8
## debug Node.js program
* [Node.js Debugging Guide](https://nodejs.org/en/docs/guides/debugging-getting-started/)
* [--inspect-brk vs. --inspect --debug-brk in newer Node.js versions](https://github.com/nodejs/help/issues/955)
* [How To Pause Your Code With Breakpoints In Chrome DevTools](https://developers.google.com/web/tools/chrome-devtools/javascript/breakpoints)
* [Chrome DevTools: Show native functions in JS Profile](http://www.mattzeunert.com/2016/08/06/show-native-functions-in-js-profile.html)

## [v8](https://chromium.googlesource.com/v8/v8.git)
* [v8 Documentation](https://v8.dev/docs)
* [Checking out the V8 source code](https://v8.dev/docs/source-code)
* [Background compilation](https://v8.dev/blog/background-compilation)
* [Embedded builtins](https://v8.dev/blog/embedded-builtins)
* [Ignition](https://v8.dev/docs/ignition)
* [Understanding V8’s Bytecode](https://medium.com/dailyjs/understanding-v8s-bytecode-317d46c94775)
* [Node.js V8 internals: an illustrative primer](https://codeburst.io/node-js-v8-internals-an-illustrative-primer-83766e983bf6)
* [How JavaScript works: inside the V8 engine + 5 tips on how to write optimized code](https://blog.sessionstack.com/how-javascript-works-inside-the-v8-engine-5-tips-on-how-to-write-optimized-code-ac089e62b12e)
* [v8-sandbox](https://github.com/thlorenz/v8-sandbox)
* [Javascript native wrappers in V8](https://medium.com/@hyperandroid/javascript-native-wrappers-in-v8-part-i-67851a3a797a)
* [Calling a v8 javascript function from c++ with an argument](https://stackoverflow.com/questions/11387015/calling-a-v8-javascript-function-from-c-with-an-argument)
* [Understanding How the Chrome V8 Engine Translates JavaScript into Machine Code](https://medium.freecodecamp.org/understanding-the-core-of-nodejs-the-powerful-chrome-v8-engine-79e7eb8af964)

* [编译 V8 源码](https://zhuanlan.zhihu.com/p/25120909)
* [使用 D8 分析 javascript 如何被 V8 引擎优化的](https://zhuanlan.zhihu.com/p/25122691)
* [v8引擎是如何知道js数据类型的？](https://www.zhihu.com/question/62732293/answer/201723301)

